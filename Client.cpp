...
bool __fastcall Hooked_CreateMove(PVOID ClientMode, int edx, float input_sample_frametime, CUserCmd* pCommand)
{
	...
	try
	{
		...
		gBacktracking.Run(pLocal, pCommand); //Backtrack (can cause crashes)
		if (GAME_TF2)
	...
	if (GAME_TF2 && Stage == FRAME_NET_UPDATE_POSTDATAUPDATE_START)
	{
		...
		for (auto i = 1; i <= gInts.Engine->GetMaxClients(); i++)
		{
			...
			if (gCvars.aimbot_resolver)
			{
				...
				if (gCvars.aimbot_resolver2)
				{
					*WriteYaw = -90.0f;
			    }

				if (gCvars.aimbot_resolver3)
				{
					*WriteYaw = 90.0f;
				}

				if (gCvars.aimbot_resolver4)
				{
					*WriteYaw = -180.0f;
				}

				if (gCvars.aimbot_resolver5)
				{
					*WriteYaw = -360.0f;
				}
			}
